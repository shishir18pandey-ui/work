...
APPS_CONFIG: Dict[str, AppConfig] = {
    "cbs": AppConfig(
        ...
        db_instance="main",
        services=["cbs-backend", "cbs-api", "cbs-core"],
        tables=["GLDM", "INFZ", "NPAR", "ACD3", "STTM", "DPD", "ACCOUNT"],
    ...
    "optimus": AppConfig(
        ...
        db_instance="idp",
        services=["optimus-api", "optimus-login", "optimus-web", "optimus-mobile"],
        tables=["users", "devices", "sessions", "oauth_tokens", "mfa_records"],
    ...
    "idp": AppConfig(
        ...
        db_instance="idp",
        services=["idp-auth", "idp-oauth", "idp-mfa"],
        tables=["users", "oauth_tokens", "mfa_records", "devices", "sessions"],
...
then this i snew file : within same folder 

ddiscovery_tool.pyimport os
import httpx
...
JAEGER_API_BASE = os.getenv("JAEGER_API_BASE", "https://tracing.prod-opt.idfcfirstbank.com/api")
...
from tools.service_metadata import (
    get_services as _get_services,
    get_service_tags as _get_service_tags,
    get_service_purpose as _get_service_purpose,
    get_app_config as _get_app_config,
    get_elk_indexes as _get_elk_indexes,
)
...
def get_services_impl(app: str) -> str:
    ...
    purposes = []
    for svc in services:
        purpose = _get_service_purpose(app, svc)
        if purpose:
...
def get_service_tags_impl(app: str, service: str) -> str:
    ...
    tags = _get_service_tags(app, service)
...
async def get_elk_fields_impl(app: str, index_type: str = "log") -> str:
    ...
    elk_indexes = _get_elk_indexes(app)
